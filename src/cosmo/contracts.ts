 ...
 * - Frontend: src/cosmo/contracts.ts (THIS FILE)
 * - Backend: supabase/functions/_shared/cosmo/contracts.ts (AUTHORITATIVE)
 * 
 * Backend is AUTHORITATIVE - frontend types should be compatible subsets.
 * When updating types, update backend first, then sync frontend.
 * 
 * Last Sync Verified: 2025-12-15
 * 
 * COSMO Canonical Contracts
 * 
 * THE ONE AND ONLY CONTRACT STANDARD FOR COSMO
 * 
 * 6 HARD RULES:
 * 1. One contract standard only: This file contains the ONLY valid COSMO types
 * 2. UI types live in src/types/ui/ (not in COSMO core)
 ...
 * 6. No new type files: All future COSMO types go here or they don't exist
 * 
 * PIPELINE CONSTITUTION:
 * 1. Request received (any source)
 * 2. Adapter normalizes → NormalizedRequest
 * 3. CosmoContext constructed
 * 4. governance.preflight(context)
 * 5. intent.detect(context) → intent
 * 6. planning.build(context) → plan steps
 * 7. execution.run(context, plan) → tool/model calls
 * 8. governance.postflight(result)
...
// SYNC CHECK: PreMessageConfig must match supabase/functions/_shared/cosmo/contracts.ts exactly

...
export function ScheduledJobsTab() {
  ...
  const getJobIcon = (jobName: string) => {
    if (jobName.includes('sync')) return <RefreshCw className="h-5 w-5 text-blue-500" />;
    if (jobName.includes('cleanup')) return <Trash2 className="h-5 w-5 text-destructive" />;
    if (jobName.includes('orphan')) return <FolderX className="h-5 w-5 text-orange-500" />;
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                      <h4 className="font-medium text-foreground">{job.job_name}</h4>

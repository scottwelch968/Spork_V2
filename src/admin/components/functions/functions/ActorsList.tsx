...
import { Card, CardContent, CardHeader, CardTitle, Table, TableBody, TableCell, TableHead, TableHeader, TableRow, Badge, Button, Switch, Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger, Input, Label, Textarea, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Tabs, TabsContent, TabsList, TabsTrigger, Checkbox, ScrollArea, AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/admin/ui';
...
const displayModeColors: Record<string, string> = {
  ui: 'bg-admin-success/10 text-admin-success border-admin-success/20',
  minimal: 'bg-admin-warning/10 text-admin-warning border-admin-warning/20',
  silent: 'bg-admin-bg-muted text-admin-text-muted border-admin-border/50',
};
    ...
    <Card>
      <CardHeader className="pb-3 flex flex-row items-center justify-between">
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="text-sm font-bold font-roboto-slab uppercase tracking-wider">Create New Actor</DialogTitle>
            </DialogHeader>
            <ActorForm
              functions={functions || []}
              onSubmit={(data) => {
            ...
            {actors?.map((actor) => (
              <TableRow key={actor.id}>
                ...
                <TableCell>
                  <Badge
                    variant="outline"
                    className={cn("text-[10px] font-bold uppercase tracking-wider", displayModeColors[actor.default_display_mode])}
                  >
                ...
                <TableCell>
                  <div className="flex flex-wrap gap-1">
                    {actor.allowed_functions.slice(0, 3).map((func) => (
                      <code
                        key={func}
                        className="text-[10px] bg-admin-secondary/10 text-admin-secondary-text px-1.5 py-0.5 rounded font-mono border border-admin-secondary/20 uppercase font-bold tracking-wider"
                      >
                        {func}
                      </code>
                    ))}
                ...
                <TableCell>
                  <div className="flex items-center gap-1">
                    <Dialog>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle className="text-sm font-bold font-roboto-slab uppercase tracking-wider">Edit Actor: {selectedActor?.name}</DialogTitle>
                        </DialogHeader>
                        {selectedActor && (
                          <ActorForm
                            actor={selectedActor}
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              ...
                              className="bg-admin-error text-white font-bold uppercase tracking-wider text-[10px] hover:bg-admin-error/90"
                            >
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <Tabs defaultValue="basic" className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-admin-bg-muted/50 p-1 border border-admin-border/50">
          <TabsTrigger value="basic" className="text-[10px] font-bold uppercase tracking-widest data-[state=active]:bg-admin-bg data-[state=active]:text-admin-accent">Basic</TabsTrigger>
          <TabsTrigger value="sequence" className="text-[10px] font-bold uppercase tracking-widest data-[state=active]:bg-admin-bg data-[state=active]:text-admin-accent">Sequence</TabsTrigger>
          <TabsTrigger value="context" className="text-[10px] font-bold uppercase tracking-widest data-[state=active]:bg-admin-bg data-[state=active]:text-admin-accent">Context</TabsTrigger>
        </TabsList>
          ...
          {formData.function_sequence.length > 0 ? (
            <div className="space-y-2 border rounded-md p-3">
              {formData.function_sequence.map((step, index) => (
                <div
                  key={index}
                  className="flex items-center gap-2 p-2 bg-admin-bg-muted rounded border border-admin-border"
                >
                  <span className="text-xs text-admin-text-muted w-6">
                    {index + 1}.
                  </span>
                  <code className="text-sm font-mono flex-1">{step.function}</code>
                  <div className="flex items-center gap-1">
                    <Button
                      type="button"
                      variant="ghost"
            ...
            <div className="text-center py-6 text-admin-text-muted text-sm border border-admin-border rounded-md">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1">
              {formData.allowed_functions
                .filter(
                  (f) => !formData.function_sequence.some((s) => s.function === f)
                )
                .map((funcKey) => (
                  <Badge
                    key={funcKey}
                    variant="outline"
                    className="cursor-pointer hover:bg-admin-accent/10"
        ...
        <TabsContent value="context" className="space-y-4 pt-4">
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-admin-text-muted">
              Default context values passed to functions for this actor.
            </p>
          </div>
        </TabsContent>
      </Tabs>

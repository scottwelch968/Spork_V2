...
import { RadioGroup, RadioGroupItem } from '@/admin/ui/radio-group';
              ...
              {formData.scheduleType === 'custom' ? (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="font-mono"
                  />
                  <p className="text-xs text-admin-text-muted">Format: minute hour day month weekday</p>
                </div>
              ) : (
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="p-3 bg-admin-bg-muted/50 rounded-lg">
              <div className="text-sm font-medium text-admin-text-muted">Cron Expression</div>
              <code className="text-lg font-mono">{getCronExpression() || '—'}</code>
            </div>

            <div className="p-3 bg-admin-bg-muted/50 rounded-lg">
              <div className="text-sm font-medium text-admin-text-muted">Schedule Description</div>
              <div className="text-lg">{getScheduleDescription() || '—'}</div>
            </div>

            <div className="border border-admin-border rounded-lg p-4">
              <div className="text-sm font-medium text-admin-text-muted mb-3 flex items-center gap-2">
              ...
              <ul className="space-y-2">
                {getNextExecutions().map((date, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm">
                    <span className="w-6 h-6 rounded-full bg-admin-accent/10 text-admin-accent flex items-center justify-center text-xs">
                      {i + 1}
                    </span>

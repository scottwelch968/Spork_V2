...
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from '@/admin/ui/dialog';
...
export const TemplateEditor = ({ open, onOpenChange, template, onSave }: TemplateEditorProps) => {
  ...
  const sanitizedHtmlContent = useMemo(() => {
    if (!formData.html_content) {
      return '<p class="text-admin-text-muted">HTML content will appear here</p>';
    }
        ...
        <Tabs defaultValue="basic" className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {formData.variables.length === 0 && (
                  <p className="text-sm text-admin-text-muted">Variables will be extracted automatically from your content</p>
                )}
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="content" className="space-y-4">
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-admin-text-muted">Use {`{{variable_name}}`} for dynamic content</p>
            </div>
          ...
          <TabsContent value="preview" className="space-y-4">
            ...
            <div className={`border border-admin-border rounded-lg overflow-hidden ${previewMode === 'mobile' ? 'max-w-md mx-auto' : ''}`}>
              <div className="bg-admin-bg-muted p-3 border-b border-admin-border">
                <p className="text-sm font-medium">{formData.subject_template || 'Subject line preview'}</p>
              </div>
              <div 
                className="bg-admin-bg p-6 overflow-auto max-h-[400px]"

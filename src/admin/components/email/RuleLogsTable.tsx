            ...
            {logs.length === 0 ? (
              ...
              logs.map((log) => (
                <>
                  <TableRow 
                    key={log.id}
                    className="cursor-pointer hover:bg-admin-bg-muted/50"
                  ...
                  {expandedLog === log.id && (
                    <TableRow>
                      <TableCell colSpan={6} className="bg-admin-bg-muted/30">
                        <div className="space-y-2 py-2">
                          {log.error_message && (
                            <div className="flex items-start gap-2">
                              <AlertCircle className="h-4 w-4 text-admin-error mt-0.5" />
                              <div>
                                <p className="font-medium text-sm">Error</p>
                                <p className="text-sm text-admin-text-muted">{log.error_message}</p>
                              </div>
                            </div>
                          )}
                          <div>
                            <p className="font-medium text-sm mb-1">Event Payload:</p>
                            <pre className="text-xs bg-admin-bg p-2 rounded overflow-auto max-h-32">

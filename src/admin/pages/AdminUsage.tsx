...
import { Activity, ArrowUp, ArrowDown, ArrowUpDown, Calendar as CalendarIcon } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-roboto-slab font-bold text-admin-text">System Usage</h1>
          <p className="text-sm text-admin-text-muted">Track system activity and token consumption</p>
        </div>
      </div>
      ...
      <Card className="bg-admin-bg-elevated border-admin-border">
        <CardHeader>
          ...
          <CardDescription className="text-admin-text-muted">Last 200 usage logs</CardDescription>
                ...
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      !startDate && 'text-admin-text-muted'
                    )}
                  >
                ...
                <TableBody>
                  {paginatedLogs.length === 0 ? (
                    ...
                    paginatedLogs.map((log) => (
                      <TableRow key={log.id} className="border-admin-border">
                        <TableCell className="text-sm">
                          {new Date(log.created_at).toLocaleString()}
                        </TableCell>
                        <TableCell>
                          <div className="text-sm">
                            <div className="font-medium text-admin-text">{log.profiles?.first_name ? `${log.profiles.first_name} ${log.profiles.last_name || ''}`.trim() : 'N/A'}</div>
                            <div className="text-admin-text-muted text-xs">{log.profiles?.email}</div>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-sm text-right text-admin-text">
                          {log.tokens_used?.toLocaleString() || 0}
                        </TableCell>
                        <TableCell className="text-sm text-right text-admin-text font-medium">

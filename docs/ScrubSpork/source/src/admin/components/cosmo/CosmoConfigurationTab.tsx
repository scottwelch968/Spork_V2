...
import { Card, CardContent, CardDescription, CardHeader, CardTitle, Button, Label, Textarea, Switch, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Slider, Collapsible, CollapsibleContent, CollapsibleTrigger } from '@/admin/ui';
import { useSystemSettings } from '@/hooks/useSystemSettings';
import { useModels } from '@/hooks/useModels';
import { toast } from 'sonner';
import { Loader2, Zap, Save, ChevronDown, DollarSign, Sparkles, Crown, Brain, Wand2, FileText } from 'lucide-react';
          ...
          <CollapsibleTrigger className="w-full">
            <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <Switch checked={routingEnabled} onCheckedChange={setRoutingEnabled} onClick={(e) => e.stopPropagation()} />
            ...
            <CardContent className="space-y-6 pt-0">
              <div className="space-y-2">
                ...
                <Select value={routingModelId} onValueChange={setRoutingModelId}>
                  <SelectTrigger><SelectValue placeholder="Select an OpenRouter model" /></SelectTrigger>
                  <SelectContent>
                    {openRouterModels.map((model) => (
                      <SelectItem key={model.id} value={model.model_id}>{model.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-4">
                ...
                <Slider value={[costWeight]} onValueChange={(value) => setCostWeight(value[0])} min={0} max={100} step={1} />
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>Lowest Cost</span><span>Balanced</span><span>Best Quality</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setRoutingPrompt(DEFAULT_ROUTING_PROMPT)}>Reset</Button>
                </div>
                <Textarea value={routingPrompt} onChange={(e) => setRoutingPrompt(e.target.value)} rows={8} className="font-mono text-sm" />
              </div>
            </CardContent>
          </CollapsibleContent>
        </Card>
          ...
          <CollapsibleTrigger className="w-full">
            <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <Switch checked={enhanceEnabled} onCheckedChange={setEnhanceEnabled} onClick={(e) => e.stopPropagation()} />
            ...
            <CardContent className="space-y-6 pt-0">
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">Include persona, personal context, and history</p>
                </div>
                <Switch checked={usePreMessageContext} onCheckedChange={setUsePreMessageContext} />
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label>Enhancement System Prompt</Label>
                  <Button variant="ghost" size="sm" onClick={() => setEnhancePrompt(DEFAULT_ENHANCE_PROMPT)}>Reset</Button>
                </div>
                <Textarea value={enhancePrompt} onChange={(e) => setEnhancePrompt(e.target.value)} rows={8} className="font-mono text-sm" />
              </div>
            </CardContent>
          </CollapsibleContent>
        </Card>
          ...
          <CollapsibleTrigger className="w-full">
            <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <Switch checked={formattingEnabled} onCheckedChange={setFormattingEnabled} onClick={(e) => e.stopPropagation()} />
          ...
          <CollapsibleContent>
            <CardContent className="space-y-4 pt-0">
              <div className="space-y-2">
                ...
                <Textarea value={formattingRules} onChange={(e) => setFormattingRules(e.target.value)} rows={10} placeholder="Enter formatting guidelines..." className="font-mono text-sm" />
                <p className="text-sm text-muted-foreground">These rules will be added to the system prompt for all chat responses.</p>
              </div>
            </CardContent>
          </CollapsibleContent>
        </Card>
